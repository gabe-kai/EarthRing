...
/**
 * Delete all procedural structures and trigger chunk regeneration
 * Admin-only endpoint
 * @returns {Promise<Object>} Response with structures_deleted and chunks_deleted counts
 */
export async function deleteAllProceduralStructures() {
  ...
  const response = await fetch(getAPIURL('/api/structures/all/procedural'), {
    method: 'DELETE',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      ...
      // Retry request with new token
      const newToken = getAccessToken();
      const retryResponse = await fetch(getAPIURL('/api/structures/all/procedural'), {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${newToken}`,
          'Content-Type': 'application/json',
    ...
    if (response.status === 403) {
      let errorMessage = 'Admin access required';
      try {
        const error = await response.json();
        errorMessage = error.message || error.error || errorMessage;
      } catch (err) {
      ...
      throw new Error(errorMessage + '. Please log out and log back in if you recently received admin privileges.');
    }
