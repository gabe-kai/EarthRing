...
- **Left industrial zone**: 20m wide strip on the negative Y side (left side) of the restricted zone
- **Right industrial zone**: 20m wide strip on the positive Y side (right side) of the restricted zone
...
**Function:** `generate_chunk_restricted_zone(floor: int, chunk_index: int) -> dict`
