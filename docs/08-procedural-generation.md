...
- **Current**: Returns chunks with ring floor geometry, station flares, and default restricted zones
- **Per-Chunk Zones**: ✅ **IMPLEMENTED** - Each chunk includes one default restricted zone (1000m length × 20m width) for maglev transit
- **Future**: Full generation with buildings, player zones, structures (Phase 2)
...
- **Dimensions**:
  - Width: 20 meters on each side of the restricted zone
  - Length: Extends to the start of station flares (25km for pillar/elevator hubs)
  - **North industrial zone**: 20m wide strip on the negative Y side (north) of the restricted zone
  - **South industrial zone**: 20m wide strip on the positive Y side (south) of the restricted zone
...
- **North industrial zone**: 20m wide strip on the negative Y side (north side) of the restricted zone
- **South industrial zone**: 20m wide strip on the positive Y side (south side) of the restricted zone
- Industrial zones follow the variable width of the restricted zone, maintaining a 20m width at all points
...
- **North commercial zone**: 10m long (X axis) × 20m wide (Y axis) on the north side of restricted zone
- **South commercial zone**: 10m long (X axis) × 20m wide (Y axis) on the south side of restricted zone
- Commercial zones are positioned at the center of each chunk (5m on each side of chunk center)
- These zones create 10m × 20m commercial areas breaking up the industrial zones every chunk (1km intervals)
- Commercial zones touch the restricted zone on their inner edge and are within the industrial zones
- These zones are system zones (protected from player modifications) and marked with `metadata.hub_zone = true`
...
- **North commercial zone**: 10m long (X axis) × 20m wide (Y axis) on the north side of restricted zone
- **South commercial zone**: 10m long (X axis) × 20m wide (Y axis) on the south side of restricted zone
- Positioned at the center of each chunk (5m on each side of chunk center)
- Creates 10m × 20m commercial zones breaking up the industrial zone every chunk (1km intervals)
- Commercial zones touch the restricted zone on their inner edge and extend 20m outward into the industrial zone
- These zones are system zones (protected from player modifications) and marked with `metadata.hub_zone = true` and `metadata.side = "north"` or `"south"`
...
- **Player Modifications**: 
  - `is_system_zone = false` - These zones are NOT protected
  - Players can dezone agricultural zones (even if unowned)
  - Players can replace agricultural zones by creating overlapping zones (new zones win)
...
- `generate_chunk_industrial_zones(floor: int, chunk_index: int) -> list`
  - Generates two 20m-wide industrial zones on either side of the restricted zone
...
- `generate_chunk_commercial_zones(floor: int, chunk_index: int) -> list`
  - Generates two 10m × 20m commercial zones at chunk centers
...
- ✅ Per-chunk restricted zone generation implemented (one default restricted zone per chunk)
