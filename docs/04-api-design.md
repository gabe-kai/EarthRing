...
Administrative endpoints for managing zones and chunks. Requires authentication and lower rate limits (10 requests per minute per user) due to the destructive nature of these operations.

#### Get Zone Count

Returns the total number of zones in the database.

```
GET /api/admin/zones/count
Headers: Authorization: Bearer <access_token>
```
...
Deletes all zones from the database. Use with caution - this operation is irreversible.

```
DELETE /api/admin/zones/reset
Headers: Authorization: Bearer <access_token>
```
...
Deletes all chunks from the database. Chunks will be regenerated by the procedural service on next request. Use with caution - this operation is irreversible.

```
DELETE /api/admin/chunks/reset
Headers: Authorization: Bearer <access_token>
```
...
**Note**: These endpoints are available via the admin modal in the web client (click "Admin" button in user info bar when authenticated).
...
- Version 6: Phase 2 - Changed to 4m floor system (1-5 floors) with new window types (full-height, standard, ceiling)
- Version 7: Phase 2 - Enhanced building shape weights: industrial (short/wide/long, fewer windows, multiple garage doors), commercial (5-story office towers, floor-to-ceiling windows, doors on all sides), residential (apartments/campuses/houses with varied door counts), agricultural clusters (house+barn+industrial), park buildings (small scattered structures)
