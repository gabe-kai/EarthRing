...
2. **Restricted Zones (Per-Chunk)**
   ...
   - **Purpose**: Prevent or limit procedural generation in specific areas (maglev transit corridor)
   - **Dimensions**: 
...
3. **Agricultural Zones (Default)**
   - **Distribution**: Generated in free space between stations (outside station flare areas)
   - **Purpose**: Default agricultural areas that can be modified by players
   - **Dimensions**: 
     - Length: Full chunk length (1000 meters)
     - Width: Variable - fills space from restricted zone edges to chunk edges
   - **Generation**: Automatically generated by procedural service for chunks outside station flares
   - **Persistence**: Stored in database and linked to chunks via `chunk_data.zone_ids`
   - **Lifecycle**: Zones appear/disappear with their associated chunks
   - **Metadata**: Marked with `metadata.default_zone = 'true'` and `metadata.chunk_index`
   - **Player Modifications**: 
     - `is_system_zone = false` - NOT protected from player modifications
     - Players can dezone agricultural zones (even if unowned)
     - Players can replace agricultural zones by creating overlapping zones
     - Agricultural zones can be modified or removed by players

4. **Atlas Pillar Zones**
