...
**Zone-Chunk Binding**: Zones are embedded within chunk data rather than being sent separately. Each chunk in the `chunks` array contains a `zones` array with zones associated with that chunk. The client extracts zones from chunk data using `ChunkManager.extractZonesFromChunk()` and passes them to the zone manager. This ensures zones appear/disappear with their associated chunks, providing seamless integration with the chunk streaming system.

**Zone Storage**: Zones are stored in the `zones` table and linked to chunks via `chunk_data.zone_ids` array. When chunks are generated by the procedural service, default restricted zones are created and stored. When chunks are loaded from the database, zones are retrieved by ID and embedded in the chunk data sent to the client.

**Zone Lifecycle**: 
- Zones are created during chunk generation by the procedural service
- Zones are stored in the database with metadata (`default_zone = 'true'`, `chunk_index`)
- Zones are loaded when chunks are streamed to the client
- Zones are extracted from chunk data and rendered by the zone manager
- Zones are cleaned up when chunks are unloaded (via `ZoneManager.cleanupZonesForChunk()`)
