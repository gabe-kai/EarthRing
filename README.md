...
- **Procedural building generation**: Buildings are procedurally generated in industrial, commercial, mixed-use, residential, and agricultural zones at hub platforms. Buildings have varied footprints (10-80m width/depth) and discrete heights (5, 10, 15, or 20m within each 20m level). Building subtypes include warehouses, factories, residences, agri-industrial, retail, and mixed-use, each with appropriate dimensions and window density. Buildings include simple window patterns (density varies by subtype) and are validated to stay within zone boundaries. Structures persist with chunks and reload when chunks are loaded.
- **Construction & demolition animations**: Buildings construct over time with visual animations (vertical reveal effect). Construction state is tracked in the database and synchronized via WebSocket. Buildings grow from the ground up over a configurable duration (default 5 minutes). Demolition animations shrink, rotate, and fade out buildings. All animations work seamlessly without page refreshes. See [Building Construction Animations Documentation](docs/implementations/building-construction-animations.md) for details.
...
3. **Configure environment variables**
   ...
   **Server configuration:**
   ...
   # - JWT_SECRET (generate with: [System.BitConverter]::ToString((1..32 | ForEach-Object {Get-Random -Maximum 256})).Replace("-", "").ToLower())
